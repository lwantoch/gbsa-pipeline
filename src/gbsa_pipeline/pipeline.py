...
def _stage_load_membrane_system(config: RunConfig, stage_dir: Path) -> Any:
    ...
    For a GROMACS topology (config.membrane_system.is_amber_format is False),
    ...
    function's docstring. This needs a real GROMACS install.

    Loading a GROMACS system fixed this way still doesn't guarantee later MD
    stages will succeed: BioSimSpace/Sire's own topology writer has a
    separate, unrelated data-loss bug (drops [nonbond_params]/[pairtypes]
    override tables on re-serialization) that can break minimization for
    force fields that rely on them, such as MemProtMD's default
    GROMOS+OPLS-lipid output — see the gbsa_pipeline.membrane module
    docstring for the full investigation. That is a BioSimSpace/Sire
    limitation, not something this function (or this pipeline) can fix.

    An AMBER topology (.prmtop/.parm7 + .inpcrd/.rst7) needs neither step and
    hits neither issue — confirmed end-to-end (load, minimize) against a real
    Lipid21-parametrized membrane protein.
    """
    ...
    if membrane.is_amber_format:
        logger.info(
            "  structure=%s  topology=%s (AMBER — no canonicalization needed)",
            membrane.structure,
            membrane.topology,
        )
        ...
        logger.info(
            "  structure=%s (canonicalized → %s)  topology=%s",
            membrane.structure,
            canonical_gro.name,
            membrane.topology,
        )
